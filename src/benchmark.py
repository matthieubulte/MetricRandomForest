                ...
                for subsample_frac in subsample_fracs:
                    for mtry_frac in mtry_fracs:
                        for i in range(replicas):
                            ...
                            mtry = max(1, int(p * mtry_frac))
                            params = dict(i=i, N=N, p=p, min_split_size=min_split_size, subsample_frac=subsample_frac, mtry_frac=mtry_frac)

                            print(f'[{str(datetime.now())}] {params}')

                            params['method']='cart_2means'
                            cart_2means = BaggedRegressor(
                                Tree(impurity_method='cart', split_type='2means', is_honest=is_honest, min_split_size=min_split_size, mtry=mtry),
                            ...
                            medoid_2means = BaggedRegressor(
                                Tree(impurity_method='medoid', split_type='2means', is_honest=is_honest, min_split_size=min_split_size, mtry=mtry),
                                n_estimators=n_trees,
                                bootstrap_fraction=subsample_frac
                            )
                            ...
                            medoid_greedy = BaggedRegressor(
                                Tree(impurity_method='medoid', split_type='greedy', is_honest=is_honest, min_split_size=min_split_size, mtry=mtry),
                                n_estimators=n_trees,
                                bootstrap_fraction=subsample_frac
                            )
